			...
			if err == nil && d.shouldOverride(ctx, result, sniffingRequest, destination) {
				...
				if sniffingRequest.RouteOnly && protocol != "fakedns" && protocol != "fakedns+others" && !isFakeIP {
					...
					if sniffingRequest.OverrideIp && !ob.OriginalTarget.Address.Family().IsDomain() {
						ips, _, err := d.dns.LookupIP(domain, dns.IPOption{
							IPv4Enable: ob.OriginalTarget.Address.Family().IsIPv4(),
							IPv6Enable: ob.OriginalTarget.Address.Family().IsIPv6(),
							FakeEnable: false,
						})
						if len(ips) == 0 || err != nil {
							errors.LogWarning(ctx, "failed to resolve domain:", domain, ", Falling back to client-requested IP:", ob.OriginalTarget.Address.String())
						} else {
							originalIP := ob.OriginalTarget.Address.IP()
							...
							if selectedIP == nil {
								selectedIP = ips[dice.Roll(len(ips))]
								errors.LogDebug(ctx, "no original IP in DNS results, randomly selected:", selectedIP)
							} else {
								errors.LogDebug(ctx, "using original client IP from DNS results:", selectedIP)
							}

							destination.Address = net.IPAddress(selectedIP)
							ob.Target = destination
						}
					}
				} else {
